...
contract ProjectReaderImpl is IProjectRead, ProjectStorage {
    ...
    {
        return (totalReferralSubscriptions, totalReferralRewardsDistributed, totalPendingReferralRewards);
    }
        ...
        returns (uint256 withdrawableAmount, uint256 totalBalance, uint256 reservedForReferrals)
    {
        ...
        withdrawableAmount = totalBalance > reservedForReferrals ? totalBalance - reservedForReferrals : 0;
    }
        ...
        returns (address[] memory subscribers, DataTypes.UserSubscription[] memory subscriptions, uint256 total)
    {
        ...
        if (offset >= total) {
            return (new address[](0), new DataTypes.UserSubscription[](0), total);
        }
